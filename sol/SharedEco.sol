...
contract SharedEco {
    ...
    function VerifyProposal(uint id) public returns (uint,string memory) {
        //msg.value = 2;
    ...
    function ExecuteContract(uint cid) payable public {
        //require(msg.value==contracts.getAmount(cid)&& msg.sender==contracts.getLoaner(cid));
    }
    function ConfirmEndOfContract(uint id) public returns(uint){
        uint ownerIncome = contracts.getAmount(id);
        //if (msg.sender ==contracts.getOwnerAddr(id)){

            msg.sender.transfer(ownerIncome);
    ...
    function getBuyerIncome(uint id) public view returns(uint){
        return contracts.getAmount(id);
    }
